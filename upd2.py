...
def create_mail_tm_account():
    ...
    domain_resp = session.get("https://api.mail.tm/domains").json()
    domain = domain_resp['hydra:member'][0]['domain']
    username = ''.join(random.choices("abcdefghijklmnopqrstuvwxyz1234567890", k=10))
    email = f"{username}@{domain}"
    password = "Astrong!Passw0rd"

    account_payload = {
        "address": email,
        "password": password
    }
    session.post("https://api.mail.tm/accounts", json=account_payload)
    token_resp = session.post("https://api.mail.tm/token", json=account_payload).json()
    session.headers.update({"Authorization": f"Bearer {token_resp['token']}"})
    return {
        "email": email,
        "password": password,
        "session": session
